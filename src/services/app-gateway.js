...
REG._svcDefs['Application Gateway'] = {
  ...
  _applyStepVisibility: function(r){ if (REG['_agw_applyStepVisibility']) REG['_agw_applyStepVisibility'](r); },
};

// SKU(제품군)에 따라 청구 항목 옵션을 교체하고, 현재 값이 새 목록에 없으면 첫 항목으로 기본 설정
REG['_agw_applyStepVisibility'] = function(r) {
...
REG['_buildDetail_Application_Gateway'] = function(r) {
  var o = r.options || {};
...
REG['_resolve_Application_Gateway'] = async function(row, cur) {
  var o = row.options || {};
  var sku = o.sku || '';
  var cfg = _AGW_CFG[sku];
  if (!cfg) {
