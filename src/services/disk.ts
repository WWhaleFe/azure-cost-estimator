...
//   3. PREMIUM_SSD_RI_SUPPORTED REG 전역 노출
// ================================================================

var DISK_SUBTYPE_MAP: Record<string, any> = REG.DISK_SUBTYPE_MAP = {
...
var PREMIUM_SSD_RI_SUPPORTED = REG.PREMIUM_SSD_RI_SUPPORTED = new Set(['P30','P40','P50','P60','P70','P80']);

var ULTRA_DISK_SIZES = REG.ULTRA_DISK_SIZES = [
  { gib:4,     label:'4 GiB' },
  { gib:8,     label:'8 GiB' },
  { gib:16,    label:'16 GiB' },
  { gib:32,    label:'32 GiB' },
...
var DISK_CATALOG: Record<string, any[]> = REG.DISK_CATALOG = {
...
// _buildDetail_Disk
// ================================================================
REG['_buildDetail_Disk'] = function(r: Row) {
  const o = r.options;
...
// _resolve_Disk — 진입점
// ================================================================
REG['_resolve_Disk'] = async function(row: Row, cur: string) {
  const o = row.options||{};
  const sub = DISK_SUBTYPE_MAP[o.diskSubType];
  if (!sub) {
...
// ================================================================
async function _resolveStandardDisk(row: Row, cur: string, diskType: string) {
  const o = row.options||{};
  const sub = DISK_SUBTYPE_MAP[o.diskSubType];
  ...
  try {
    ...
    const isPlain = function(it: ApiItem){ const m=(it.meterName||'').toLowerCase(); return !m.includes('burst')&&!m.includes('enablement')&&!m.includes('snapshot')&&!m.includes('one-time')&&!m.includes('encrypt')&&!m.includes('shared')&&!m.includes('confidential'); };
...
// ================================================================
async function _resolvePremiumSSD(row: Row, cur: string) {
  const o = row.options||{};
  ...
  try {
    ...
    const isPlain=function(it: ApiItem){ const m=(it.meterName||'').toLowerCase(); return !m.includes('mount')&&!m.includes('burst')&&!m.includes('enablement')&&!m.includes('snapshot')&&!m.includes('one-time')&&!m.includes('encrypt')&&!m.includes('shared')&&!m.includes('confidential'); };
    const exp=(skuFull+' Disk').toLowerCase();
    const disk = diskItems.filter(function(it){
      if((it.type||'').toLowerCase()!=='consumption') return false;
      if(!(it.unitOfMeasure||'').toLowerCase().includes('month')) return false;
...
// ================================================================
async function _resolveProvisionedDisk(row: Row, cur: string, diskType: string) {
  const o = row.options||{};
  ...
  const matchesType = function(it: ApiItem) {
