...
REG._svcDefs['Azure Synapse Analytics'] = {
  ...
  _applyStepVisibility: function(r: Row){ if (REG['_synapse_applyStepVisibility']) REG['_synapse_applyStepVisibility'](r); },
};

REG['_synapse_applyStepVisibility'] = function(r: Row) {
...
REG['_buildDetail_Azure_Synapse_Analytics'] = function(r: Row) {
  var o = r.options || {};
...
REG['_resolve_Azure_Synapse_Analytics'] = async function(row: Row, cur: string) {
  var o = row.options || {};
  var comp = o.component || 'Dedicated SQL Pool (DWU)';
  var product: string, label: string, matcher: (it: ApiItem) => boolean;

  if (comp.indexOf('Dedicated') >= 0) {
    var dwu = o.dwuLevel || _SYN_DWU[0];
    product = 'Azure Synapse Analytics Dedicated SQL Pool';
    label = 'Synapse / Dedicated SQL Pool ' + dwu;
    matcher = function(it: ApiItem){
    ...
    matcher = function(it: ApiItem){ return String(it.meterName||'').toLowerCase() === 'vcore'; };
  } else {
    product = 'Azure Synapse Analytics Serverless SQL Pool';
    label = 'Synapse / Serverless SQL Pool';
    matcher = function(it: ApiItem){ return String(it.meterName||'').toLowerCase().indexOf('data processed') >= 0; };
  }
  ...
  if (comp.indexOf('Dedicated') >= 0) {
    var resv: ApiItem[] = [];
    try { resv = await apiFetch({ serviceName:'Azure Synapse Analytics', armRegionName:row.region, productName:product, priceType:'Reservation' }, cur, 200, 3, {pageSize:200, expectedSizeKB:40}); } catch(e: any) { resv = []; }
