...
var _APPSVC_SIZES: Record<string,string[]> = {
  'Free':        ['F1'],
  'Shared':      ['Shared'],
  'Basic':       ['B1','B2','B3'],
  'Standard':    ['S1','S2','S3'],
...
REG._svcDefs['App Service'] = {
  ...
  _applyStepVisibility: function(r: Row){ if (REG['_appsvc_applyStepVisibility']) REG['_appsvc_applyStepVisibility'](r); },
};

REG['_appsvc_applyStepVisibility'] = function(r: Row) {
  var def = REG._svcDefs['App Service'];
...
REG['_buildDetail_App_Service'] = function(r: Row) {
  var o = r.options || {};
  REG['_appsvc_applyStepVisibility'](r);
  r.skuName = o.size || '';
  r.detail = [o.tier, o.os, o.size].filter(Boolean).join(' / ');
};

// 공백·대소문자 무시 정규화 ('P1 v3' → 'p1v3') — 인스턴스 표기 차이를 흡수
function _appsvc_norm(s: any){ return String(s||'').toLowerCase().replace(/\s+/g,''); }

// 가격 조회 — productName(계층+OS) + skuName=인스턴스, 절약 플랜/예약 포함
REG['_resolve_App_Service'] = async function(row: Row, cur: string) {
  var o = row.options || {};
  ...
  var cItems: ApiItem[] = [], rItems: ApiItem[] = [];
  try {
  ...
  var cons = cItems.filter(function(it: ApiItem){ return String(it.type||'').toLowerCase()==='consumption'; });
  var cands = cons.filter(function(it: ApiItem){ return _appsvc_norm(it.skuName)===sizeNm; })
  ...
  var sp1: ApiItem|null=null, sp3: ApiItem|null=null;
  var exSp=function(item: any){
  ...
  var anySpInCands = cands.some(function(c: ApiItem){ return Array.isArray(c.savingsPlan) && c.savingsPlan.length>0; });

  // 예약(1·3년): skuName/armSkuName/meterName(공백무시)으로 인스턴스 매칭
  var wantLinux = (os === 'Linux');
  var riMatch = function(arr: ApiItem[]){
    return arr.filter(function(it: ApiItem){
  ...
  if (riBase.length === 0) {
    ...
    } catch (e: any) { /* 폴백 실패는 무시 */ }
  }
  var osPref = function(arr: ApiItem[]){
    var m = arr.filter(function(it: ApiItem){ return /linux/i.test(String(it.productName||'')) === wantLinux; });
    return m.length ? m : arr; // OS 구분이 없으면 전체 사용
  };
  var pickTerm = function(rx: RegExp){
    var arr = osPref(riBase.filter(function(it: ApiItem){ return rx.test(String(it.reservationTerm||'')); }));
    arr.sort(function(a: ApiItem, b: ApiItem){ return Number(a.unitPrice||0) - Number(b.unitPrice||0); });
    return arr[0] || null;
  };
  ...
  console.log('[AppSvc] 진단', {
    ...
    reservationSample: rItems.slice(0,10).map(function(x: ApiItem){ return { sku:x.skuName, arm:x.armSkuName, term:x.reservationTerm, prod:x.productName }; }),
