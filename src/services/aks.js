...
REG._svcDefs['Azure Kubernetes Service'] = {
  ...
  _applyStepVisibility: function(r){ if (REG['_aks_applyStepVisibility']) REG['_aks_applyStepVisibility'](r); },
};

// 계층에 따라 SLA 옵션 표시/숨김 토글 (Standard일 때만 노출)
REG['_aks_applyStepVisibility'] = function(r) {
...
// detail 빌더
REG['_buildDetail_Azure_Kubernetes_Service'] = function(r) {
  var o = r.options || {};
...
REG['_aks_pickGradeHourly'] = function(items, grade) {
  var cands = [];
...
// 가격 조회
REG['_resolve_Azure_Kubernetes_Service'] = async function(row, cur) {
  var o = row.options || {};
  var tier = o.aksTier || 'Standard (표준)';
  var isStandard = tier.indexOf('Standard') === 0 || tier.indexOf('표준') >= 0;
