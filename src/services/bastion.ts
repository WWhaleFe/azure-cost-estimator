...
REG['_buildDetail_Azure_Bastion'] = function(r: Row) {
  const o = r.options || {};
  r.skuName = o.tier || '';
...
REG['_resolve_Azure_Bastion'] = async function(row: Row, cur: string) {
  const o = row.options || {};
  ...
  let cands = items.filter((it: ApiItem) => {
  ...
  if (isTransfer) {
    const paid = cands.filter((it: ApiItem) => Number(it.unitPrice||0) > 0);
    if (paid.length) cands = paid;
  }
  cands.sort((a: ApiItem, b: ApiItem)=> (Number(a.tierMinimumUnits||0) - Number(b.tierMinimumUnits||0)) || (Number(a.unitPrice||0) - Number(b.unitPrice||0)));
