...
var _REDIS_TIERS: Record<string, any> = {
...
REG._svcDefs['Azure Cache for Redis'] = {
  ...
  _applyStepVisibility: function(r: Row){ if (REG['_redis_applyStepVisibility']) REG['_redis_applyStepVisibility'](r); },
};

// 계층에 따라 캐시 크기 옵션을 교체하고, 현재 값이 새 목록에 없으면 비운다
REG['_redis_applyStepVisibility'] = function(r: Row) {
...
REG['_buildDetail_Azure_Cache_for_Redis'] = function(r: Row) {
  var o = r.options || {};
...
REG['_resolve_Azure_Cache_for_Redis'] = async function(row: Row, cur: string) {
  var o = row.options || {};
  ...
  cands.sort(function(a: ApiItem, b: ApiItem){
