...
import type { Row, ApiItem, SpPair, RiPair } from '../core/kernel.js';
// ================================================================
...
var _SQL_PRODUCT: Record<string,string> = {
...
var _SQL_COMPUTE: Record<string,string[]>  = { 'General Purpose':['Provisioned','Serverless'], 'Business Critical':['Provisioned'], 'Hyperscale':['Provisioned','Serverless'] };
var _SQL_HARDWARE: Record<string,string[]> = { 'General Purpose':['Gen5','Fsv2-series'], 'Business Critical':['Gen5','M-series'], 'Hyperscale':['Gen5','Premium-series','Premium-series MO','DC-series'] };
var _SQL_RED_ZR = '영역 중복(ZR)';
var _SQL_RED_LOCAL = '로컬 중복';

// ── DTU 구매 모델(단일 DB) ── productName=계층, skuName=DTU 크기, 단위 '1/Day'(일일 단가).
var _SQL_DTU_PRODUCT: Record<string,string> = { 'Basic':'SQL Database Single Basic', 'Standard':'SQL Database Single Standard', 'Premium':'SQL Database Single Premium' };
var _SQL_DTU_SIZES: Record<string,string[]>   = { 'Basic':['B'], 'Standard':['S0','S1','S2','S3','S4','S6','S7','S9','S12'], 'Premium':['P1','P2','P4','P6','P11','P15'] };
var _SQL_VCORE_TIERS = ['General Purpose','Business Critical','Hyperscale'];
var _SQL_DTU_TIERS   = ['Basic','Standard','Premium'];

// ── SQL 라이선스(AHB) ── Retail Prices API의 'vCore' 단가는 '컴퓨팅 전용(=AHB)' 가격이고,
//   계산기 기본값 '라이선스 포함'은 여기에 SQL Server 코어 라이선스를 더한 값이다.
//   라이선스 단가는 Retail API에 미제공(별도 미터 없음)이라 Azure 공시값을 상수로 둔다(USD/vCore/h, 리전 무관):
//     GP=SQL Standard 코어(≈$0.10), BC=SQL Enterprise 코어(≈$0.375), Hyperscale=라이선스 없음.
//   통화 환산은 같은 컴퓨팅 미터의 USD↔선택통화 비율(FX)을 API에서 도출해 적용(상수는 USD만).
//   계산기 East US GP/BC 2vCore 분해로 검증: GP 라이선스 150.04 KRW/vCore/h, BC 562.84 KRW/vCore/h.
var _SQL_LICENSE_USD: Record<string,number> = { 'General Purpose': 0.10, 'Business Critical': 0.375, 'Hyperscale': 0 };
...
var _SQL_STORAGE: Record<string, any> = {
...
REG._svcDefs['Azure SQL Database'] = {
  ...
  rebuildKeys: ['model'],
  _applyStepVisibility: function(r: Row){ if (REG['_sql_applyStepVisibility']) REG['_sql_applyStepVisibility'](r); },
};

REG['_sql_applyStepVisibility'] = function(r: Row) {
  var def = REG._svcDefs['Azure SQL Database'];
...
REG['_buildDetail_Azure_SQL_Database'] = function(r: Row) {
  var o = r.options || {};
  REG['_sql_applyStepVisibility'](r);
  if (o.model === 'DTU') {
    r.skuName = o.dtuSize || '';
...
async function _resolve_sql_dtu(row: Row, cur: string) {
  var o = row.options || {};
...
async function _sql_storage_monthly(row: Row, cur: string, tier: string, isZR: boolean, storageGB: number) {
  ...
  } catch (e: any) { return { monthly:0, rate:0, note:'스토리지 조회 실패' }; }
  var isCons  = function(it: ApiItem){ return String(it.type||'').toLowerCase() === 'consumption'; };
  var meterEq = function(it: ApiItem, m: string){ return String(it.meterName||'').toLowerCase() === m; };
  var target  = (isZR && sm.meterZR) ? sm.meterZR : sm.meter;
  var pick = function(m: string){
    return sItems.filter(function(x: ApiItem){ return isCons(x) && meterEq(x, m); })
...
REG['_resolve_Azure_SQL_Database'] = async function(row: Row, cur: string) {
  var o = row.options || {};
  ...
  var isCons    = function(it: ApiItem){ return String(it.type||'').toLowerCase() === 'consumption'; };
  var meterIs   = function(it: ApiItem, m: string){ return String(it.meterName||'').toLowerCase() === m; };
  var skuIs     = function(it: ApiItem, s: string){ return String(it.skuName||'').toLowerCase() === s; };

  // 항목 합산: 로컬 base 항목 a에 ZR add-on 항목 b의 단가를 더한 새 항목(시간당) 생성
  function addItems(a: ApiItem | null, b: ApiItem | null, lab: string){
    if (!a) return null;
    if (!b) return a;
  ...
  function computePrice(meterLower: string, exactSku: string, perVcoreSkus: string[]){
    var exact = items.filter(function(it: ApiItem){ return isCons(it) && meterIs(it, meterLower) && skuIs(it, exactSku); })
                     .sort(function(a: ApiItem, b: ApiItem){ return Number(a.unitPrice||0) - Number(b.unitPrice||0); })[0];
    ...
    for (var i = 0; i < perVcoreSkus.length && !base; i++) {
      base = items.filter(function(it: ApiItem){ return isCons(it) && meterIs(it, meterLower) && skuIs(it, perVcoreSkus[i]); })[0];
    }
  ...
  // 3) SQL 라이선스(AHB) — '라이선스 포함'이면 SQL Server 코어 라이선스를 컴퓨팅에 더함.
  //    'vCore' 단가는 컴퓨팅 전용(=AHB)이므로 AHB 선택 시 addon 0. 라이선스 단가는 USD 상수 → API에서 FX 도출해 환산.
  var licMode = o.license || _SQL_LIC_INCLUDED;
  var licUSDrate = (licMode === _SQL_LIC_INCLUDED) ? (_SQL_LICENSE_USD[tier] || 0) : 0;
  var licAddon = 0;
  if (licUSDrate > 0 && N > 0) {
    var perVcoreCur = local.price / N;          // 컴퓨팅 per-vCore (선택 통화)
    var perVcoreUSD = perVcoreCur;              // USD면 동일
    if (String(cur).toUpperCase() !== 'USD') {
      try {
        var usdItems = await apiFetch({ serviceName:'SQL Database', armRegionName:row.region, productName:product, priceType:'Consumption' }, 'USD', 300, 4, {pageSize:200, expectedSizeKB:120});
        var ub = usdItems.filter(function(it: ApiItem){ return isCons(it) && meterIs(it,'vcore') && (skuIs(it,'vcore')||skuIs(it,'1 vcore')); })[0];
        var ue = usdItems.filter(function(it: ApiItem){ return isCons(it) && meterIs(it,'vcore') && skuIs(it, N+' vcore'); })[0];
        if (ub) perVcoreUSD = Number(ub.unitPrice);
        else if (ue) perVcoreUSD = Number(ue.unitPrice) / N;
      } catch(e: any) { /* FX 실패 시 perVcoreCur 사용(근사) */ }
    }
  ...
  function addFlat(it: ApiItem | null){ if(!it || flatAddon<=0) return it; var p=Number(it.unitPrice)+flatAddon; return Object.assign({}, it, {unitPrice:p, retailPrice:p, _sqlLicAddon:licAddon, _sqlStoHourly:stoHourly}); }
  ...
  function spBaseFor(meterLower: string, exactSku: string, perVcoreSkus: string[]){
    var b = null;
    for (var i = 0; i < perVcoreSkus.length && !b; i++) {
      b = items.filter(function(it: ApiItem){ return isCons(it) && meterIs(it, meterLower) && skuIs(it, perVcoreSkus[i]); })[0];
    }
    if (b) return { base:b, mult:N };
    var ex = items.filter(function(it: ApiItem){ return isCons(it) && meterIs(it, meterLower) && skuIs(it, exactSku); })[0];
  ...
  var spZR: SpPair = { sp1:null, sp3:null };
  if (isZR && !zrMissing) {
  ...
  var resv: ApiItem[] = [];
  try { resv = await apiFetch({ serviceName:'SQL Database', armRegionName:row.region, productName:product, priceType:'Reservation' }, cur, 200, 3, {pageSize:200, expectedSizeKB:60}); } catch(e: any) { resv = []; }
  // 견고화: 예약 제공 조합인데 비어있으면 1회 재시도(일시적 프록시 실패 대비)
  if (riExpected && (!resv || resv.length === 0)) {
    try { resv = await apiFetch({ serviceName:'SQL Database', armRegionName:row.region, productName:product, priceType:'Reservation' }, cur, 200, 3, {pageSize:200, expectedSizeKB:60}); } catch(e: any) { resv = []; }
  }
  var riLocal = riItemsFromResv(resv, 'vcore', N, cur);
  var riZR: RiPair = (isZR && !zrMissing) ? riItemsFromResv(resv, 'vcore zr zone redundancy', N, cur) : { ri1:null, ri3:null };
