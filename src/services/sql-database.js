...
var _SQL_LICENSE_USD = { 'General Purpose': 0.10, 'Business Critical': 0.375, 'Hyperscale': 0 };
...
REG._svcDefs['Azure SQL Database'] = {
  ...
  rebuildKeys: ['model'],
  _applyStepVisibility: function(r){ if (REG['_sql_applyStepVisibility']) REG['_sql_applyStepVisibility'](r); },
};

REG['_sql_applyStepVisibility'] = function(r) {
...
REG['_buildDetail_Azure_SQL_Database'] = function(r) {
  var o = r.options || {};
...
REG['_resolve_Azure_SQL_Database'] = async function(row, cur) {
  var o = row.options || {};
  ...
  if (licUSDrate > 0 && N > 0) {
    ...
    if (String(cur).toUpperCase() !== 'USD') {
      ...
      } catch(e) { /* FX 실패 시 perVcoreCur 사용(근사) */ }
    }
