...
REG._svcDefs['Azure Cosmos DB'] = {
  ...
  _applyStepVisibility: function(r: Row){ if (REG['_cosmos_applyStepVisibility']) REG['_cosmos_applyStepVisibility'](r); },
};

// RU/s 입력은 Provisioned/Autoscale에서만 사용
REG['_cosmos_applyStepVisibility'] = function(r: Row) {
  var def = REG._svcDefs['Azure Cosmos DB'];
...
REG['_buildDetail_Azure_Cosmos_DB'] = function(r: Row) {
  var o = r.options || {};
  REG['_cosmos_applyStepVisibility'](r);
  var model = o.model || '';
  var needsRu = model.indexOf('Provisioned') === 0 || model.indexOf('Autoscale') === 0;
...
REG['_resolve_Azure_Cosmos_DB'] = async function(row: Row, cur: string) {
  var o = row.options || {};
