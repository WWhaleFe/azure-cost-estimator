...
REG['_buildDetail_VPN_Gateway'] = function(r: Row) {
  const o = r.options;
  r.skuName = o.sku || '';
  const parts = [];
...
REG['_resolve_VPN_Gateway'] = async function(row: Row, cur: string) {
  const o=row.options||{};
  ...
  let allItems: ApiItem[]=[],gateway: ApiItem|null=null,s2sItem: ApiItem|null=null,p2sItem: ApiItem|null=null,vnetItem: ApiItem|null=null,steps: string[]=[],errors: string[]=[];
  try{ allItems=await apiFetch({serviceName:'VPN Gateway',armRegionName:row.region,priceType:'Consumption'},cur,200,3); }
  catch(err: any){ row.paygItem=null;row.sp1Item=null;row.sp3Item=null;row.ri1Item=null;row.ri3Item=null; setStatus('error',`VPN 조회 실패: ${err.message.slice(0,80)}`); updatePriceCells(row);updateTotalsRow();return; }
  const norm=(s: any)=>String(s||'').toLowerCase().replace(/\s+/g,''),skuN=norm(sku);
  try{
    const isE=(it: ApiItem)=>{ const m=(it.meterName||'').toLowerCase(); return m.includes('tunnel')||m.includes('s2s')||m.includes('p2s')||m.includes('connection')||m.includes('data transfer')||m.includes('inter-'); };
    const gwC=allItems.filter((it: ApiItem)=>{ if((it.type||'').toLowerCase()!=='consumption') return false; if(!(it.unitOfMeasure||'').toLowerCase().includes('hour')) return false; if(isE(it)) return false; const mN=norm(it.meterName),sN=norm(it.skuName); return mN===skuN||sN===skuN||mN.startsWith(skuN)||sN.startsWith(skuN)||mN.endsWith(skuN)||sN.endsWith(skuN); });
    gwC.sort((a: ApiItem, b: ApiItem)=>{ const ae=(norm(a.meterName)===skuN||norm(a.skuName)===skuN)?0:1,be=(norm(b.meterName)===skuN||norm(b.skuName)===skuN)?0:1; if(ae!==be)return ae-be; return Number(b.unitPrice||0)-Number(a.unitPrice||0); });
    gateway=gwC[0]||null;
  }catch(err: any){errors.push(`GW:${err.message}`);}
  if(eS2s>0){try{ const c=allItems.filter((it: ApiItem)=>{ if((it.type||'').toLowerCase()!=='consumption') return false; if(!(it.unitOfMeasure||'').toLowerCase().includes('hour')) return false; const m=(it.meterName||'').toLowerCase(); if(m.includes('p2s')||m.includes('point-to-site')) return false; return m.includes('s2s')||m.includes('site-to-site')||m.includes('tunnel'); }).sort((a: ApiItem, b: ApiItem)=>Number(a.unitPrice||0)-Number(b.unitPrice||0)); s2sItem=c[0]||null; }catch(err: any){errors.push(`S2S:${err.message}`);}}
  if(eP2s>0){try{ const c=allItems.filter((it: ApiItem)=>{ if((it.type||'').toLowerCase()!=='consumption') return false; if(!(it.unitOfMeasure||'').toLowerCase().includes('hour')) return false; const m=(it.meterName||'').toLowerCase(); if(m.includes('s2s')||m.includes('site-to-site')||m.includes('tunnel')) return false; return m.includes('p2s')||m.includes('point-to-site')||m.includes('connection'); }).sort((a: ApiItem, b: ApiItem)=>Number(a.unitPrice||0)-Number(b.unitPrice||0)); p2sItem=c[0]||null; }catch(err: any){errors.push(`P2S:${err.message}`);}}
  // Zone 매핑 — 리전별 인터넷 이그레스 존 번호
  const ZM: Record<string,number>={eastus:1,eastus2:1,westus:1,westus2:1,westus3:1,northcentralus:1,southcentralus:1,centralus:1,westcentralus:1,westeurope:1,northeurope:1,francecentral:1,francesouth:1,uksouth:1,ukwest:1,canadacentral:1,canadaeast:1,koreacentral:2,koreasouth:2,japaneast:2,japanwest:2,eastasia:2,southeastasia:2,australiaeast:2,australiasoutheast:2,centralindia:2,southindia:2,westindia:2,qatarcentral:2,brazilsouth:3,brazilsoutheast:3,southafricanorth:3,southafricawest:3,uaenorth:3,uaecentral:3,israelcentral:3};
  const uZ=ZM[row.region]||1,tt=o.vnetTransferType||'VNET 간';
  const exGb=(items: ApiItem[])=>items.filter((it: ApiItem)=>{ if((it.type||'').toLowerCase()!=='consumption') return false; const u=(it.unitOfMeasure||'').toLowerCase(); return u.includes('gb')&&!u.includes('hour'); });
  ...
  const isVnetOut=(it: ApiItem)=>{ const a=`${it.meterName||''} ${it.productName||''} ${it.skuName||''}`.toLowerCase(); const out=a.includes('outbound')||a.includes('egress')||/\bout\b/.test(a); const vnetish=a.includes('inter-virtual network')||a.includes('inter virtual network')||a.includes('inter-vnet')||a.includes('inter vnet')||a.includes('vnet to vnet')||a.includes('vnet-to-vnet')||a.includes('virtual network peering')||a.includes('vnet peering'); if(vnetish&&out) return true; if(a.includes('peering')&&out) return true; if(a.includes('outbound')&&(a.includes('vnet')||a.includes('virtual network'))) return true; return false; };
  const isVpnE=(it: ApiItem)=>{ const a=`${it.meterName||''} ${it.productName||''} ${it.skuName||''}`.toLowerCase(); if(a.includes('inter-virtual network')||a.includes('inter-vnet')||a.includes('peering')||a.includes('inter-region')||a.includes('cross region')) return false; return a.includes('vpn')||a.includes('data transfer'); };
  const fbT=(gbC: ApiItem[],type: string)=>type==='VNET 간'?gbC.filter((it: ApiItem)=>Number(it.unitPrice||0)>0&&isVnetOut(it)):gbC.filter((it: ApiItem)=>isVpnE(it));
  const srtC=(cands: ApiItem[])=>{ const zr=new RegExp(`zone\\s*${uZ}\\b`,'i'),ck=(it: ApiItem)=>zr.test(it.meterName||'')||zr.test(it.skuName||'')||zr.test(it.productName||''); return cands.slice().sort((a: ApiItem, b: ApiItem)=>{ const az=ck(a)?0:1,bz=ck(b)?0:1; if(az!==bz)return az-bz; const ao=/\bout\b|outbound/i.test(a.meterName||'')?0:1,bo=/\bout\b|outbound/i.test(b.meterName||'')?0:1; if(ao!==bo)return ao-bo; return Number(a.unitPrice||0)-Number(b.unitPrice||0); }); };
  const mkId=(it: any)=>it.meterId||`${it.serviceName}|${it.armRegionName}|${it.meterName}|${it.unitPrice}`;
  if(vnetGB>0){
    let am: ApiItem[]=[];
    try{ const c1=fbT(exGb(allItems),tt); if(c1.length>0){am=am.concat(c1);steps.push(`VPN ${c1.length}건`);}else{const gp=exGb(allItems).filter((it: ApiItem)=>Number(it.unitPrice||0)>0);if(gp.length>0){am=am.concat(gp);steps.push(`VPN GB폴백 ${gp.length}건`);}} }catch(err: any){errors.push(`VNETA:${err.message}`);}
    if(am.length===0){try{const vn=await apiFetch({serviceName:'Virtual Network',armRegionName:row.region,priceType:'Consumption'},cur,500,3,{pageSize:200,expectedSizeKB:300});const c2=fbT(exGb(vn),tt);if(c2.length>0){am=am.concat(c2);steps.push(`VN ${c2.length}건`);}else steps.push(`VN 0/${vn.length}`);}catch(err: any){steps.push('VN실패');}}
    if(am.length===0){try{const bw=await apiFetch({serviceName:'Bandwidth',armRegionName:row.region,priceType:'Consumption'},cur,2000,5,{pageSize:500,expectedSizeKB:800});const c3=fbT(exGb(bw),tt);if(c3.length>0){am=am.concat(c3);steps.push(`BW ${c3.length}건`);}else steps.push(`BW 0/${bw.length}`);}catch(err: any){steps.push('BW실패');}}
    const uq=new Map();am.forEach((it: ApiItem)=>{const id=mkId(it);if(!uq.has(id))uq.set(id,it);});const vc=srtC(Array.from(uq.values()));if(vc.length>0)vnetItem=vc[0];else if(tt==='VPN')vnetItem={meterName:`[zone${uZ} 무료]`,skuName:'Free',unitPrice:0,retailPrice:0,unitOfMeasure:'1 GB',currencyCode:cur,productName:'VPN intra',serviceName:'VPN Gateway'};
  }
