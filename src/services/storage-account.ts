...
var _STG_PRODUCT: Record<string,string> = { 'Table': 'Tables', 'Queue': 'Queues v2' };
// 청구 항목 → 종류별 meterName 끝말(소문자). [Table, Queue] 순
var _STG_METER: Record<string, { kw?: string; table?: string; queue?: string; exclude: string[] }> = {
...
REG['_buildDetail_Storage_Account'] = function(r: Row) {
  var o = r.options || {};
...
REG['_resolve_Storage_Account'] = async function(row: Row, cur: string) {
  var o = row.options || {};
  ...
  var kw: any = spec.kw || (stype === 'Queue' ? spec.queue : spec.table);
