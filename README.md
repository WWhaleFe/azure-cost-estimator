...
- 정렬 중인 열은 헤더 아래쪽 **노란 강조선**과 **▲▼** 로만 표시합니다(헤더 배경을 덮지 않아 글자가 그대로 읽힙니다).
- 화면 상단에 **`현재 'Qty' 열 오름차순으로 보는 중`** 이 정렬을 푸는 순간까지 **계속** 떠 있습니다.
- 정렬은 **보기 순서만** 바꿉니다. 원본 입력 순서는 그대로 보관되므로 상단 **"↺ 원본 데이터 순으로 보기"** 버튼이나 같은 헤더 3번째 클릭으로 언제든 정확히 되돌아갑니다.
