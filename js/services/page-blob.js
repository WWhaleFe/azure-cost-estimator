...
window._svcDefs['Page Blob'] = {
  ...
  _applyStepVisibility: function(r){ if (window['_pageblob_applyStepVisibility']) window['_pageblob_applyStepVisibility'](r); },
};

window['_pageblob_applyStepVisibility'] = function(r) {
  var def = window._svcDefs['Page Blob'];
...
window['_buildDetail_Page_Blob'] = function(r) {
  var o = r.options || {};
  window['_pageblob_applyStepVisibility'](r);
...
window['_resolve_Page_Blob'] = async function(row, cur) {
  var o = row.options || {};
  var isPrem = (o.performance === 'Premium');
  ...
  if (isPrem) {
    ...
    chosen = items.filter(function(it){ return isCons(it) && String(it.skuName||'').toLowerCase() === skuT && String(it.meterName||'').toLowerCase().indexOf('disk') >= 0; })
    ...
    chosen = items.filter(function(it){
      ...
      return false;
    }).sort(function(a,b){ var ta=Number(a.tierMinimumUnits||0),tb=Number(b.tierMinimumUnits||0); if(ta!==tb) return ta-tb; return Number(a.unitPrice||0)-Number(b.unitPrice||0); })[0] || null;
