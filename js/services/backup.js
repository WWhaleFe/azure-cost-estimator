...
window._svcDefs['Backup'] = {
  ...
  _applyStepVisibility: function(r){ if (window['_backup_applyStepVisibility']) window['_backup_applyStepVisibility'](r); },
};

// 청구 항목에 따라 옵션 표시/숨김: 보호 인스턴스→워크로드만, 백업 저장소→저장소 계층+중복성만
window['_backup_applyStepVisibility'] = function(r) {
  var def = window._svcDefs['Backup'];
...
window['_buildDetail_Backup'] = function(r) {
  var o = r.options || {};
  var metric = o.metric || '보호 인스턴스';
  window['_backup_applyStepVisibility'](r);
  var isInstance = metric.indexOf('보호') >= 0;
  if (isInstance) {
...
window['_resolve_Backup'] = async function(row, cur) {
  var o = row.options || {};
  var metric = o.metric || '보호 인스턴스';
  var isInstance = metric.indexOf('보호') >= 0;
