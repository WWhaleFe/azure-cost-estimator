...
//         표준 SLA와 동일한 미터로 API 조회(Automatic 전용 관리요금 미터는 비공개).
...
window['_aks_applyStepVisibility'] = function(r) {
  var def = window._svcDefs['Azure Kubernetes Service'];
...
window['_buildDetail_Azure_Kubernetes_Service'] = function(r) {
  ...
  window['_aks_applyStepVisibility'](r);
  var isStandard = tier.indexOf('Standard') === 0 || tier.indexOf('표준') >= 0;
  var parts = ['클러스터 관리'];
  if (isStandard) {
...
//   grade: 'standard'(표준 SLA) 또는 'premium'(LTS)
//   standard: 'Standard'/'Uptime'/'SLA' 계열, 'Premium'/'LTS'/'Long Term' 제외
//   premium : 'Premium' 또는 'LTS'/'Long Term' 포함
window['_aks_pickGradeHourly'] = function(items, grade) {
  var wantPremium = (grade === 'premium');
  var cands = [];
...
window['_resolve_Azure_Kubernetes_Service'] = async function(row, cur) {
  ...
  //   Automatic                        → standard 미터(표준 SLA와 동일하게 청구)
  var grade = 'standard';
  if (isStandard && sla.indexOf('Long Term') >= 0) grade = 'premium';
  ...
  var chosen = window['_aks_pickGradeHourly'](items, grade);
  ...
  if (!chosen) {
    try {
      ...
      chosen = window['_aks_pickGradeHourly'](all, grade);
