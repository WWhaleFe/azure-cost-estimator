...
window._svcDefs['Azure Cache for Redis'] = {
  ...
  _applyStepVisibility: function(r){ if (window['_redis_applyStepVisibility']) window['_redis_applyStepVisibility'](r); },
};

// 계층에 따라 캐시 크기 옵션을 교체하고, 현재 값이 새 목록에 없으면 비운다
window['_redis_applyStepVisibility'] = function(r) {
  var def = window._svcDefs['Azure Cache for Redis'];
...
window['_buildDetail_Azure_Cache_for_Redis'] = function(r) {
  var o = r.options || {};
  window['_redis_applyStepVisibility'](r);
  r.skuName = o.sku || '';
  r.detail = ['Redis', o.tier, o.sku].filter(Boolean).join(' / ');
};

// 가격 조회 — productName=계층 제품 + skuName 정확 일치, meter '<SKU> Cache' 우선
window['_resolve_Azure_Cache_for_Redis'] = async function(row, cur) {
  var o = row.options || {};
