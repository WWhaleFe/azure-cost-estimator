...
// buildSkuAndDetail: 각 서비스 파일의 _buildDetail 함수를 호출
function buildSkuAndDetail(r) {
  const def = SERVICE_CATEGORIES[r.serviceCategory];
  if (!def) return;
  // 해당 서비스 파일에 _buildDetail 함수가 있으면 우선 사용
...
// tryResolveItem: 서비스별 resolve 함수 호출
async function tryResolveItem(row) {
  ...
  // 전용 resolve 함수 이름 규칙: resolve_<카테고리>
